...
int in_bluetooth_connect_read(struct bluetooth_inverter * inv) {
    ...
    struct timeval tv;
    tv.tv_sec = 5;     //set timeout of reading
    tv.tv_usec = 0;

    FD_ZERO(&readfds);
    FD_SET(inv->socket_fd, &readfds);

    select(inv->socket_fd+1, &readfds, NULL, NULL, &tv);	

    if (FD_ISSET(inv->socket_fd, &readfds))       // did we receive anything within BT_TIMEOUT seconds
        count = read(inv->socket_fd, inv->buffer, BUFSIZ);
    else
    {
		log_info("Timeout reading bluetooth socket");
		return -1;
    }
