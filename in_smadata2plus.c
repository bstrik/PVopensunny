...
/*
 * Test Query Day Values
 */
void in_smadata2plus_get_day_values(struct bluetooth_inverter * inv,time_t time_start,time_t time_end){
	...
	/* temp value store */
	int timestamp = 0,diff = 0,found = 0;
	long value = 0,prev_value = 0;
	int day,month,year,hour,minute;
	time_t datum;
    struct tm *loctime;
    FILE	*fp;
	fp = fopen("/tmp/opensunny.csv","w");
	fprintf(fp,";SN: %d;SN: %d\n",inv->serial,inv->serial);
	fprintf(fp,"dd-MM-yyyy HH:mm;kWh;kW\n");    
	...
	while(1){
		...
		while(pos < recv_pl2.content_length){
			...
			memcpy(&value,recv_pl2.content+pos+4,4);
			...
			if (found == 1) {			
				...
				diff *= 12;
				datum = timestamp;
                loctime = localtime(&datum);
                day = loctime->tm_mday;
                month = loctime->tm_mon +1;
                year = loctime->tm_year + 1900;
                hour = loctime->tm_hour;
                minute = loctime->tm_min; 
 				printf("%02d-%02d-%4d %02d:%02d total=%.3lf kWh power=%.3lf kW\n",day,month,year,hour,minute,(value/1000.0),(diff/1000.0));
 				fprintf(fp,"%02d-%02d-%4d %02d:%02d;%.3lf;%.3lf\n",day,month,year,hour,minute,(value/1000.0),(diff/1000.0));
			}
